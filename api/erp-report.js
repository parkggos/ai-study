...
- sections[].charts는 선택 필드. 해당 섹션 분석에 차트가 도움이 될 때만 1~2개 지정 (불필요하면 생략 또는 [])
- 사용 가능한 charts ID: monthly(월별매출), category(카테고리), channel(채널), payment(결제수단), tier(고객등급), orderStatus(주문상태), brand(브랜드TOP), region(지역TOP)
- 매출 추이·성장 분석 섹션에는 monthly, 채널·유통 섹션에는 channel, 고객·등급 섹션에는 tier, 결제·주문품질 섹션에는 payment·orderStatus 등 맥락에 맞게 배치
